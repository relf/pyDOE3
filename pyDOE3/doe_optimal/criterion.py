...
def _regularized_inv(M, reg_param=1e-8):
    """
...
def a_optimality(M):
    ...
    """
    return -float(np.trace(_regularized_inv(M)))
...
def i_optimality(M_X, moment_matrix):
    ...
    """
    return -float(np.trace(moment_matrix @ _regularized_inv(M_X)))
...
def c_optimality(M, c):
    ...
    """
    return -float(c.T @ _regularized_inv(M) @ c)
